...
const REASON_STATUS = Object.freeze({
  ...
  not_ready_for_m1: 409,
  upload_not_found: 409,
  digest_mismatch: 409,
  ambiguous_job: 409,
  needs_human: 422,
  dispatch_unconfigured: 503,
  photo_required: 400,
...
export function transitionRecord(record, event = {}) {
  ...
  switch (event.type) {
    case 'correct': {
      ...
      const merged = { ...current.seed, ...patch };
      if ('city' in patch || 'state' in patch) {
        Object.assign(merged, installLocationSeedFields(merged.city, { state: merged.state }));
      }
      const seed = safeSeed(merged);
      return {
        ok: true,
        record: {
          ...current,
    ...
    case 'handoff': {
      ...
      if (event.revision !== current.revision) return reject(current, 'stale_revision');
      if (
        current.state === INSTALL_POST_STATES.READY_FOR_M1
        && current.approval?.revision === current.revision
      ) {
        return reject(current, 'already_ready');
      }
      return {
        ok: true,
        record: {
    ...
    case 'm1_claim': {
      // The M1 worker picks up an approval that already exists; it never
      // approves. The claim is the lease, so the ordinary result callback and
      // stale-publish timeout apply unchanged.
      if (current.state !== INSTALL_POST_STATES.READY_FOR_M1) return reject(current, 'not_ready_for_m1');
      if (!current.image) return reject(current, 'photo_required');
      if (!current.approval || current.approval.revision !== current.revision) {
        return reject(current, 'stale_revision');
      }
      if (event.revision !== current.revision) return reject(current, 'stale_revision');
      if (!event.dispatchId) return reject(current, 'dispatch_mismatch');
      return {
        ok: true,
        record: {
          ...current,
          lease: {
            ...
            dispatchId: String(event.dispatchId),
            claimedAt: at,
            ...(event.workerId ? { workerId: String(event.workerId) } : {}),
          },
          result: null,
          state: INSTALL_POST_STATES.PUBLISHING,
          updatedAt: at,
