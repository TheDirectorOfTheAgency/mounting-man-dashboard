...
function sanitizeDestinations(destinations) {
  ...
  return destinations.slice(0, 12).map((entry) => ({
    id: redactUnsafeText(entry?.id || '').slice(0, 80),
    ...
    receiptId: redactUnsafeText(entry?.receiptId || '').slice(0, 160),
    receiptUrl: redactUnsafeText(entry?.receiptUrl || '').slice(0, 500),
  }));
}
...
function normalizeRecord(record = {}) {
  ...
  return {
    ...
    chatgptPreview: record.chatgptPreview || null,
...
export function transitionRecord(record, event = {}) {
  ...
  switch (event.type) {
    ...
    case 'approve': {
      ...
      let approvalNonceHash = null;
      if (event.manifestId || event.approvalNonce) {
        const preview = current.chatgptPreview;
        if (!preview) return reject(current, 'preview_required');
        approvalNonceHash = createHash('sha256').update(String(event.approvalNonce || '')).digest('hex');
        if (event.manifestId !== preview.manifestId || approvalNonceHash !== preview.approvalNonceHash) {
          return reject(current, 'approval_mismatch');
        }
        if (!Number.isFinite(Date.parse(preview.approvalExpiresAt || ''))
          || Date.parse(preview.approvalExpiresAt) <= Date.parse(at)) {
          return reject(current, 'approval_expired');
        }
        if (preview.binding?.recordRevision !== current.revision
          || preview.binding?.photoSha256 !== current.image.sha256) {
      ...
      return {
        ...
        record: {
          ...
          chatgptPreview: current.chatgptPreview ? {
            ...current.chatgptPreview,
            approvalConsumedAt: at,
            approvalDispatchId: String(event.dispatchId || ''),
          } : null,
          lease: {
            revision: current.revision,
    ...
    case 'preview': {
      ...
      if (!preview?.manifestId || !preview?.approvalNonceHash || !preview?.approvalExpiresAt
        || preview.binding?.jobId !== current.jobId
        || preview.binding?.recordRevision !== current.revision
        || preview.binding?.photoSha256 !== current.image.sha256) {
        return reject(current, 'invalid_preview');
      }
      return {
        ok: true,
        record: {
    ...
    case 'result': {
      ...
      const settled = sanitized.status === INSTALL_POST_STATES.PUBLISHED
        || sanitized.status === INSTALL_POST_STATES.INDETERMINATE;
      return {
        ok: true,
        record: {
          ...current,
          ...
          result: sanitized,
          approval: settled ? current.approval : null,
          lease: settled ? current.lease : null,
          updatedAt: at,
        },
      };
    }
