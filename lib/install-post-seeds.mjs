export const DEFAULT_TEAM_MEMBER_MAP = {
...
function decorateUnit(unit) {
  ...
  if (unit.mantelmount && !unit.fireplaceType) {
...
function fallbackSeed({ facts, order, orderId, paymentId, invoiceId, triggerStatus, triggerSourceCode, triggerEvent }) {
...
export function buildInstallPostSeeds({
  ...
  if (!tvUnits.length) {
    return [fallbackSeed({ facts, order, orderId, paymentId, invoiceId, triggerStatus, triggerSourceCode, triggerEvent })];
  }
