...
function assignWalls(units, wallItems) {
  ...
  if (surfaces.length === 1) {
    for (const unit of units) {
      if (!unitHasWallSurface(unit)) addItem(unit, { ...surfaces[0].item, amountCents: 0 });
    }
    surfaces[0].item.assigned = true;
    return;
  }

  const brick = surfaces.find((entry) => entry.surface === 'Brick');
  const fireplaceUnits = units.filter((unit) => unit.fireplaceType);
  if (brick && fireplaceUnits.length === 1) {
    addItem(fireplaceUnits[0], brick.item);
    brick.item.assigned = true;
    const remainingSurfaces = surfaces.filter((entry) => !entry.item.assigned);
    if (remainingSurfaces.length === 1) {
      for (const unit of units) {
        if (!unitHasWallSurface(unit)) addItem(unit, { ...remainingSurfaces[0].item, amountCents: 0 });
      }
      remainingSurfaces[0].item.assigned = true;
    }
  }
}
...
function decorateUnit(unit) {
  ...
  unit.wallSurface = unit.items.map((item) => detectWallSurface(itemText(item))).find(Boolean) || '';
...
function buildTvUnits(normalizedLineItems) {
  ...
  assignSequentialAccessories(units, bracketItems);
  assignSequentialAccessories(units, soundbarItems);
  assignSequentialAccessories(units, cableItems);
...
function seedFromUnit({ unit, facts, orderId, paymentId, invoiceId, triggerStatus, triggerSourceCode, triggerEvent, seedIndex, seedCount }) {
  ...
  return compactSeed({
    ...
    'wall-surface': unit.wallSurface || facts.wallSurface,
    'metro-area': '',
    'location-id': '',
