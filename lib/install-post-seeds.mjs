...
function uniqueServiceLabels(items, extraLabels = []) {
  const labels = [];
...
/**
 * One Square payment/order/invoice = one install post.
 * Multi-TV same visit collapses into a single job's facts and notes.
 */
function combineInstallUnits(units, normalized) {
  const mountUnits = units.filter((unit) => unit.jobType !== 'unmount');
  const primary = mountUnits[0] || units[0];
  const assigned = new Set(units.flatMap((unit) => unit.items));
  const leftovers = normalized
    .flatMap(expandItem)
    .filter((item) => !assigned.has(item) && !isNegatedUnmountText(itemText(item)));
  ...
  return {
    ...
    items,
    labels: uniqueServiceLabels(items, units.flatMap((unit) => unit.labels)),
...
export function buildInstallPostSeeds({
  ...
  // Locked rule: one Square job = one seed / one page / one social set.
  if (!tvUnits.length) {
