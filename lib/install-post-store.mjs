...
//   install-post:job:<jobId>     operator record (publicJobView strips Square ids)
//   install-post:source:<jobId>  Square order/payment refs, server-side only
//
// Top-level orderId/paymentId/invoiceId are stamped for bind and dedup.
// publicJobView and safeSeed still omit Square identifiers, customer name, and
// full address so a leak of the mobile surface cannot expose them.
...
// Kept in step with every save so an idle M1 poll is one SMEMBERS, not a scan.
export const M1_READY_INDEX_KEY = 'install-post:m1-ready-index';
...
function visitIdentityKey({ seed = {}, sourceRefs = {} } = {}) {
  const paymentId = firstNonEmpty(sourceRefs.paymentId, seed['source-payment-id']);
  const orderId = firstNonEmpty(sourceRefs.orderId, seed['source-order-id']);
  const invoiceId = firstNonEmpty(sourceRefs.invoiceId, seed['source-invoice-id']);
...
/**
 * Defense in depth: one Square visit = one staged job, even if a caller still
 * hands in leftover seed-index 2/3 siblings from the old fan-out.
 */
export function collapseSeedsForOneVisit(seeds, sourceRefs = {}) {
  const list = Array.isArray(seeds) ? seeds : [];
  if (list.length <= 1) {
    return list.map((seed) => (seed && typeof seed === 'object'
      ? { ...seed, 'seed-index': 1, 'seed-count': 1 }
      : seed));
  }

  const sourceKey = visitIdentityKey({ sourceRefs });
  if (sourceKey) {
    return [{ ...list[0], 'seed-index': 1, 'seed-count': 1 }];
  }

  const kept = [];
  const seen = new Set();
  for (const seed of list) {
    const key = visitIdentityKey({ seed, sourceRefs });
    if (key) {
      if (seen.has(key)) continue;
      seen.add(key);
      kept.push({ ...seed, 'seed-index': 1, 'seed-count': 1 });
      continue;
    }
    kept.push(seed);
  }
...
function newRecord({ jobId, seed, source, stagedAt, sourceRefs = {} }) {
...
/** One normalized, unapproved record per Square visit. Pure. */
export function buildJobRecords({ seeds, sourceRefs = {}, source = '', stagedAt } = {}) {
  const list = collapseSeedsForOneVisit(seeds, sourceRefs);
  const at = stagedAt || new Date().toISOString();
  return list.map((seed, index) => {
    const resolvedRefs = {
      orderId: firstNonEmpty(sourceRefs.orderId, seed?.['source-order-id']),
      paymentId: firstNonEmpty(sourceRefs.paymentId, seed?.['source-payment-id']),
      invoiceId: firstNonEmpty(sourceRefs.invoiceId, seed?.['source-invoice-id']),
    };
    const visitKey = visitIdentityKey({ seed, sourceRefs: resolvedRefs });
    return newRecord({
      jobId: installPostJobId({
        orderId: resolvedRefs.orderId,
        paymentId: resolvedRefs.paymentId,
        invoiceId: resolvedRefs.invoiceId,
        seedIndex: visitKey ? 1 : (Number(seed?.['seed-index']) || index + 1),
      }),
      seed,
      source,
      stagedAt: at,
...
export function createInstallPostStore(kv, {
  ...
  async function saveRecord(record) {
    ...
    if (record.state === INSTALL_POST_STATES.READY_FOR_M1) {
      await kv.sadd(M1_READY_INDEX_KEY, record.jobId);
    } else {
      await kv.srem(M1_READY_INDEX_KEY, record.jobId);
    }
    return record;
  }
  ...
  async function listReadyForM1Ids() {
    const members = await kv.smembers(M1_READY_INDEX_KEY);
    return Array.isArray(members) ? members : [];
  }

  /**
   * Rebuild the M1 ready index from the full job index. Needed once for jobs
   * that reached READY_FOR_M1 before the index existed; safe to repeat.
   */
  async function rebuildReadyForM1Index() {
    let ready = 0;
    for (const jobId of await listJobIds()) {
      const record = await loadRecord(jobId);
      if (record?.state === INSTALL_POST_STATES.READY_FOR_M1) {
        await kv.sadd(M1_READY_INDEX_KEY, jobId);
        ready += 1;
      } else {
        await kv.srem(M1_READY_INDEX_KEY, jobId);
      }
    }
    return ready;
  }
  ...
  /**
   * Find staged jobs whose source namespace matches paymentId and/or orderId.
   * The job record itself never stores those identifiers.
   */
  async function findRecordsBySource({ paymentId = '', orderId = '' } = {}) {
    const pay = String(paymentId || '').trim();
    const ord = String(orderId || '').trim();
    if (!pay && !ord) return [];
    const matches = [];
    for (const jobId of await listJobIds()) {
      const refs = await loadSourceRefs(jobId);
      if (pay && refs.paymentId !== pay) continue;
      if (ord && refs.orderId !== ord) continue;
      const record = await loadRecord(jobId);
  ...
  async function stageJobRecords({ seeds, sourceRefs = {}, source = '', stagedAt } = {}) {
    ...
    for (const candidate of candidates) {
      ...
      await kv.set(sourceKey(candidate.jobId), JSON.stringify({
        orderId: String(candidate.orderId || sourceRefs.orderId || ''),
        paymentId: String(candidate.paymentId || sourceRefs.paymentId || ''),
        invoiceId: String(candidate.invoiceId || sourceRefs.invoiceId || ''),
      }), { ex: ttlSeconds });
...
/**
 * Stage one cloud job per Square visit and return its operator link.
 *
