...
const CHATGPT_AUDIT_KEY_PREFIX = 'install-post:chatgpt-audit:';
const CHATGPT_REQUEST_KEY_PREFIX = 'install-post:chatgpt-request:';
...
export function createInstallPostStore(kv, {
  ...
  async function saveChatgptAudit(entry = {}) {
    const safe = {
      actorHash: /^[0-9a-f]{64}$/.test(String(entry.actorHash || '')) ? String(entry.actorHash) : '',
      tool: /^[a-z0-9_]{1,80}$/.test(String(entry.tool || '')) ? String(entry.tool) : '',
      jobId: /^job_[a-zA-Z0-9_-]{1,80}$/.test(String(entry.jobId || '')) ? String(entry.jobId) : '',
      beforeRevision: /^[0-9a-f]{64}$/.test(String(entry.beforeRevision || '')) ? String(entry.beforeRevision) : null,
      afterRevision: /^[0-9a-f]{64}$/.test(String(entry.afterRevision || '')) ? String(entry.afterRevision) : null,
      result: /^[a-z0-9_]{1,80}$/.test(String(entry.result || '')) ? String(entry.result) : 'invalid',
      requestId: /^[0-9a-f-]{36}$/.test(String(entry.requestId || '')) ? String(entry.requestId) : null,
      at: Number.isFinite(Date.parse(entry.at || '')) ? String(entry.at) : new Date().toISOString(),
    };
    if (!safe.actorHash || !safe.tool || !safe.jobId) throw new Error('invalid_chatgpt_audit');
    const digest = createHash('sha256').update(JSON.stringify(safe)).digest('hex');
    await kv.set(`${CHATGPT_AUDIT_KEY_PREFIX}${safe.jobId}:${digest}`, JSON.stringify(safe), { ex: ttlSeconds });
    return safe;
  }

  async function claimChatgptBackendRequest(requestId) {
    if (!/^[0-9a-f-]{36}$/.test(String(requestId || ''))) return false;
    try {
      return Boolean(await kv.set(`${CHATGPT_REQUEST_KEY_PREFIX}${requestId}`, '1', { nx: true, ex: 120 }));
    } catch {
      return false;
    }
  }
