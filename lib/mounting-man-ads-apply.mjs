...
export function googleAdsSearchUrl(customerId, apiVersion = ADS_API_VERSION) {
  return `${GOOGLE_ADS_API_HOST}/${apiVersion}/customers/${customerId}/googleAds:searchStream`;
}

export function googleAdsAdGroupCriteriaMutateUrl(customerId, apiVersion = ADS_API_VERSION) {
  return `${GOOGLE_ADS_API_HOST}/${apiVersion}/customers/${customerId}/adGroupCriteria:mutate`;
}

export function googleAdsCampaignCriteriaMutateUrl(customerId, apiVersion = ADS_API_VERSION) {
...
export function assertAdGroupCriterionPauseTarget(args = {}) {
  ...
  const adGroupId = digitsOnly(args.ad_group_id ?? args.adGroupId);
...
export function createMountingManAdsApplyClient({
  ...
  async function search(customerId, query) {
    const response = await adsRequest({
      method: 'POST',
      url: googleAdsSearchUrl(customerId),
      data: { query },
      write: false,
    });
  ...
  async function lookupAdGroupCriterion(customerId, criterionId) {
    const rows = await search(
      customerId,
      `SELECT ad_group_criterion.resource_name, ad_group_criterion.criterion_id, ad_group_criterion.status, ad_group_criterion.negative, ad_group_criterion.keyword.text, ad_group_criterion.keyword.match_type, campaign.id, campaign.name, ad_group.id, ad_group.name FROM ad_group_criterion WHERE ad_group_criterion.criterion_id = ${criterionId}`,
    );
    return rows[0] ? rowAdGroupCriterion(rows[0]) : null;
  }
  ...
  async function getCriterionStatus(args = {}) {
    ...
    const resourceName = String(args.resource_name || args.resourceName || '');
    const criterionId = digitsOnly(args.criterion_id ?? args.criterionId)
      || digitsOnly(resourceName.split('~').pop());
    if (!criterionId) {
      throw codedError('invalid_criterion', 'criterion_id is required');
    }
  ...
  async function pauseAdGroupCriterion(args = {}) {
    ...
    let before = await lookupAdGroupCriterion(customerId, criterionId);
    if (!before) {
      ...
      throw codedError('criterion_not_found', `No ad group criterion ${criterionId} on customer ${customerId}`);
    }
    ...
    const mutate = await adsRequest({
      method: 'POST',
      url: googleAdsAdGroupCriteriaMutateUrl(customerId),
      write: true,
      data: {
        operations: [{
          update: {
  ...
  async function addCampaignPhraseNegatives(args = {}) {
    ...
    if (toCreate.length) {
      const mutate = await adsRequest({
        method: 'POST',
        url: googleAdsCampaignCriteriaMutateUrl(customerId),
        write: true,
        data: {
