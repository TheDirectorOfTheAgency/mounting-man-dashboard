...
export function decodePngRgb(buf) {
  if (buf[0] !== 0x89 || buf[1] !== 0x50) throw new Error('recap plate is not a PNG');
  let offset = 8;
  let width = 0;
  let height = 0;
  let colorType = 2;
...
export function loadRecapPlate(platePath = PLATE_PATH) {
  if (!fs.existsSync(platePath)) return null;
  return decodePngRgb(fs.readFileSync(platePath));
}
...
export function renderRecapCardPng(text, platePath = PLATE_PATH) {
  const { amount, jobLabel, label } = parseRecapCard(text);
  const plate = loadRecapPlate(platePath);
  const width = plate?.width || RECAP_CARD_WIDTH;
  const height = plate?.height || RECAP_CARD_HEIGHT;
  const pixels = plate ? Buffer.from(plate.pixels) : Buffer.alloc(width * height * 3);
  if (!plate) {
  ...
  const amountScale = plate ? (amount.length > 10 ? 12 : 13) : (amount.length > 10 ? 14 : 16);
  blit(pixels, width, height, amount, Math.floor((width - textWidth(amount, amountScale)) / 2), plate ? 610 : 430, amountScale, GOLD);
  blit(pixels, width, height, label, Math.floor((width - textWidth(label, plate ? 6 : 8)) / 2), plate ? 800 : 620, plate ? 6 : 8, CREAM);
  blit(pixels, width, height, jobLabel, Math.floor((width - textWidth(jobLabel, plate ? 6 : 7)) / 2), plate ? 890 : 760, plate ? 6 : 7, MUTED);
  if (!plate) {
