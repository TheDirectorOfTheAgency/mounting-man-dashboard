...
//
// Applied to both the Square+photo auto-run and the manual Publish tap. This
// module only answers: is the staged city/street/size safe to publish without
// a human? HOLD reason codes are for the desk — never customer PII.

import { lookupInstallLocation } from './install-post-locations.mjs';
...
export const HOLD_REASONS = Object.freeze({
  ...
  MISSING_TV_SIZE: 'missing_tv_size',
...
/** Blank, the bare word "TV", or anything without a number is not a size. */
export function isMissingTvSize(tvSize) {
  const value = trimText(tvSize);
  if (!value) return true;
  if (/^tvs?$/i.test(value)) return true;
  return !/\d/.test(value);
}
 ...
 *
 * @param {{ city?: string, state?: string, streetName?: string, seedCount?: number, tvSize?: string }} facts
 * @returns {{ pass: boolean, reasons: string[] }}
 */
export function evaluateInstallPostConfidence({ city, state, streetName, seedCount, tvSize, ...rest } = {}) {
  void rest;
  const reasons = [];
  const cityValue = trimText(city);
  ...
  else if (!lookupInstallLocation(cityValue, { state })) reasons.push(HOLD_REASONS.UNKNOWN_CITY);
  if (isGoogleBlobStreet(streetName, cityValue)) reasons.push(HOLD_REASONS.GOOGLE_BLOB_STREET);
  if (isMissingTvSize(tvSize)) reasons.push(HOLD_REASONS.MISSING_TV_SIZE);
...
function isValidNoul(noul) {
  return noul === true || noul === false || noul === 'false' || isFiniteNumber(noul);
}
...
function noulPasses(noul) {
  ...
  if (isFiniteNumber(noul)) return noul >= JEV_CONFIDENCE_MIN;
  return false;
}
...
export async function evaluateJevInstallPostConfidence({
  ...
  try {
    ...
    if (!isValidNoul(noul)) {
      logger.warn?.('[install-post-confidence] TypeSafe Jev malformed answer — fail open', {
        errorType: 'MalformedAnswer',
      });
      return { pass: true, skipped: 'malformed_answer', reasons: [] };
    }
    const confidenceHold = isFiniteNumber(confidence) && confidence < JEV_CONFIDENCE_MIN;
