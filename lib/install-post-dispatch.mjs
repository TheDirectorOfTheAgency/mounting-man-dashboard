 ...
 *
 * Null when INSTALL_POST_DISPATCH_TOKEN is empty. Cloud Actions dispatch is
 * off by owner decision; callers treat null as "hand off to M1", so an empty
 * token must never yield an object whose dispatch() throws.
 */
export function createConfiguredDispatcher({ httpClient, env = process.env } = {}) {
  const token = (env.INSTALL_POST_DISPATCH_TOKEN || '').trim();
  if (!token) return null;
  const owner = (env.INSTALL_POST_DISPATCH_OWNER || '').trim();
  const repo = (env.INSTALL_POST_DISPATCH_REPO || '').trim();
  const workflowFile = (env.INSTALL_POST_DISPATCH_WORKFLOW || 'publish-install-post.yml').trim();
  const ref = (env.INSTALL_POST_DISPATCH_REF || 'main').trim();
