...
export function paymentTeamMemberId(payment) {
  return payment?.team_member_id || payment?.employee_id || null;
}
...
export function sanitizePaymentDetail({ payment, order, teamMember } = {}) {
  ...
  const teamMemberId = paymentTeamMemberId(source);
...
export async function getMountingManSquareDetail(rawArgs, {
  ...
  if (args.paymentId) {
    ...
    if (isCompletedAtLocation(payment, client.locationId)) {
      payments = [payment];
      date = date || chicagoDateString(payment.created_at);
    }
  } else {
  ...
  const teamIds = [...new Set(payments.map(paymentTeamMemberId).filter(Boolean))];
  ...
  const feed = {
    ...
    payments: payments.map((payment) => sanitizePaymentDetail({
      payment,
      order: orders[payment.order_id],
      teamMember: teamMembers[paymentTeamMemberId(payment)],
    })),
  };
