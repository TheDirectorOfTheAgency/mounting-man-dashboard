...
// already bound and the gate passes, approves it the same way as the phone
// tap: cloud runner if a dispatcher is configured, otherwise READY_FOR_M1.
// If the photo is missing, sends the operator upload link over a deterministic
// channel (install-post-photo-ask.mjs) — no desk wake. Woodward is woken only
// for confidence holds or when no photo-ask channel delivered. Never
// publish_one.py / go.py, never a second desk hop.
...
import { deliverPhotoAsk, readPhotoAskConfig } from './install-post-photo-ask.mjs';
import {
  buildInstallFacts,
...
export async function notifyQInstallPost(
  ...
  },
  {
    ...
    photoAskConfig = readPhotoAskConfig(),
    dispatcher,
    logger = console,
    typesafeHttpClient,
  ...
  // Photo missing → deterministic ask with the operator upload link(s).
  // Woodward is the fallback only when no photo-ask channel delivered.
  let photoAsk = { skipped: 'photo_present' };
  if (photoMissing) {
    const missingJobIds = new Set(
      recordsForJobs.filter((record) => !record.image).map((record) => record.jobId),
    );
    photoAsk = await deliverPhotoAsk({
      links: operatorLinks.filter((link) => missingJobIds.has(link.jobId)),
      httpClient,
      config: photoAskConfig,
      logger,
    });
  }
  const photoAskFallback = photoMissing && !photoAsk.delivered;
  const deskAction = photoAskFallback
    ? 'request_photo'
  ...
  // Photo present + PASS → cloud runner or READY_FOR_M1 (plain ping, no desk
  // wake). Photo ask delivered → no desk wake.
  // Confidence HOLD (or undeliverable photo ask) → wake Woodward with reason
  // codes only (no second publisher).
  let woodward = {
    skipped: photoMissing ? 'photo_ask_delivered' : 'photo_present_cloud_publisher',
  };
  if (photoAskFallback || confidenceHold) {
