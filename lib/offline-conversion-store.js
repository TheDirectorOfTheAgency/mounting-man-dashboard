...
export function createAttributionStore(kv, { ttlSeconds = DEFAULT_TTL_SECONDS } = {}) {
  ...
  const clickSessionKey = (bookingSession) =>
    `attrib:click:session:${opaqueRef(requireValue(bookingSession, 'ZenBooker booking session'))}`;
  const clickCustomerKey = (zenCustomerId) =>
    `attrib:click:customer:${opaqueRef(requireValue(zenCustomerId, 'ZenBooker customer ID'))}`;
  ...
  return {
    ...
    // Raw Google click identifier, parked against the booking it came from.
    // Written by the GCLID receiver only. No other module reads these keys yet.
    async saveClickIdentifier({ zenCustomerId, bookingSession, clickType, clickId, clickedAt = null }) {
      requireValue(zenCustomerId, 'ZenBooker customer ID');
      requireValue(bookingSession, 'ZenBooker booking session');
      requireValue(clickType, 'Click identifier type');
      requireValue(clickId, 'Click identifier');
      const value = {
        clickType,
        clickId,
        clickedAt: sanitizeTimestamp(clickedAt),
        capturedAt: new Date().toISOString(),
      };
      await Promise.all([
        kv.set(clickSessionKey(bookingSession), value, { ex: ttlSeconds }),
        kv.set(clickCustomerKey(zenCustomerId), value, { ex: ttlSeconds }),
      ]);
      return value;
    },

    async getClickIdentifier({ zenCustomerId, bookingSession } = {}) {
      if (bookingSession) {
        const bySession = await kv.get(clickSessionKey(bookingSession));
        if (bySession) return bySession;
      }
      if (zenCustomerId) return kv.get(clickCustomerKey(zenCustomerId));
      return null;
    },
