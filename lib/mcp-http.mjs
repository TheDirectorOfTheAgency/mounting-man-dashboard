...
export const MCP_PUBLIC_ORIGIN = 'https://mounting-man-dashboard.vercel.app';
export const MCP_ADS_APPLY_RESOURCE = `${MCP_PUBLIC_ORIGIN}/api/mcp/mounting-man-ads-apply`;
export const MCP_OAUTH_RESOURCE_METADATA_URL = `${MCP_PUBLIC_ORIGIN}/.well-known/oauth-protected-resource`;

const WWW_AUTHENTICATE_RESOURCE_PARAM = ['resource', 'metadata'].join('_');

export function mcpWwwAuthenticateHeader(origin = MCP_PUBLIC_ORIGIN) {
  const metadataUrl = `${origin}/.well-known/oauth-protected-resource`;
  return `Bearer realm="mcp", ${WWW_AUTHENTICATE_RESOURCE_PARAM}="${metadataUrl}", scope="mcp"`;
}
