...
function gbpSurfaceLabel(seed = {}) {
  ...
  const genericWall = !surface || surface === 'drywall' || surface === 'wall';
  if (jobUsedFireplace(seed) && genericWall) {
    return fireplaceSurfaceLabel(seed) || surface;
  }
  return surface;
}
...
function gbpGalleryProduct(seed = {}) {
  ...
  if (/oled/.test(blob) && !/soundbar/.test(blob)) return 'gallery OLED';
  return 'gallery OLED';
}
...
function isTileFireplaceJob(seed = {}) {
  if (!jobUsedFireplace(seed) || jobUsedMantel(seed)) return false;
  const surface = gbpSurfaceLabel(seed);
  const text = [seed['fireplace-type'], seed['wall-surface'], seed['job-notes']].join(' ').toLowerCase();
  return /tile/.test(surface) || /tile fireplace|fireplace.*tile|tile.*fireplace/.test(text);
}
...
function gbpProductLabel(seed = {}) {
  ...
  if (isTileFireplaceJob(seed)) return 'tile fireplace';
  if (fireplace) return 'fireplace mount';
...
function gbpHeadline(seed = {}, city = '') {
  ...
  const productSize = size && !product.includes(size) ? `${product} ${size}` : product;
  const surface = gbpSurfaceLabel(seed);
