...
function gbpSurfaceLabel(seed = {}) {
  const surface = String(seed['wall-surface'] || '').replace(/\s+/g, ' ').trim();
...
function gbpGalleryProduct(seed = {}) {
  ...
  if (brandLower.includes('g-series') || brandLower.includes('g series')) return 'LG G-Series';
  ...
  if (brandLower === 'lg') return 'LG G-Series';
...
function gbpOutcomes(seed = {}) {
  ...
  if (/center(?:ed)? on (?:the )?mantel/.test(blob)) {
    outcomes.push('Centered on mantel');
  }
  ...
  if (
    /cord conceal|cable conceal|in-wall|in wall|concealment/.test(blob)
    && !outcomes.includes('Recessed outlet')
  ) {
    outcomes.push('Cord concealment');
  }

  if (
    String(seed['fireplace-type'] || '').trim()
    && !outcomes.some((item) => /mantel|fireplace/i.test(item))
  ) {
    outcomes.push('Above fireplace');
  }
