...
import { renderRecapCardPng } from './recap-card.mjs';
...
export const UPLOAD_MEDIA_URL = 'https://upload.twitter.com/1.1/media/upload.json';
...
export function wantsRecapCard(attachImage) {
  return attachImage !== false && attachImage !== 'false';
}
...
export function createMarshallWayneXClient({
  ...
  httpClient = axios,
  oauth = {},
  renderCard = renderRecapCardPng,
} = {}) {
  async function signedRequest({ method, url, data, headers: extraHeaders, extraParams }) {
    ...
    const headers = { Authorization: header, ...extraHeaders };
    const isForm = typeof FormData !== 'undefined' && data instanceof FormData;
    if (data !== undefined && !isForm && !headers['Content-Type'] && !headers['content-type']) {
  ...
  async function uploadRecapCard(png) {
    const form = new FormData();
    form.append('media_category', 'tweet_image');
    form.append('media', new Blob([png], { type: 'image/png' }), 'recap.png');
    const response = await signedRequest({
      method: 'POST',
      url: UPLOAD_MEDIA_URL,
      data: form,
      extraParams: { media_category: 'tweet_image' },
    });
    ...
    if (!mediaId) {
      throw codedError('x_api_error', 'X API request failed: media id missing from upload response');
    }
    return mediaId;
  }

  async function postMarshallWayneRecap(text, options = {}) {
    const verbatim = recapTextOrThrow(text);
    await verifyMarshallWayne();
    const payload = { text: verbatim };
    let mediaId;
    if (wantsRecapCard(options.attach_image ?? options.attachImage)) {
      const png = renderCard(verbatim);
      mediaId = await uploadRecapCard(png);
