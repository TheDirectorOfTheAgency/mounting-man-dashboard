...
export function createRunnerCallbackHandler({ store, runnerSecret, gbpQueue, now = Date.now } = {}) {
  return async function handler(req, res) {
    ...
    const outcome = await store.withRecordLock(jobId, async (current) => {
      ...
      // Website first. Then enqueue an Update and an independent Photos-tab
      // upload for the M1 worker. Never post GBP (or Reddit) from Actions.
      ...
      try {
        ...
        record = attachGbpQueuedDestinations(record, enqueued);
      } catch (err) {
